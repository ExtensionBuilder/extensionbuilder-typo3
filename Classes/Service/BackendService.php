...
// ToDo Check
     ...
     * @since 0.12
     */
    final function composerSetup(): void
    {
        if ($this->isComposerMode) {
            $addUrl = false;
...
// ToDo
     ...
     * @since 0.12
     */
    final function getLocalExtensions(array $extensionExtensions = []): void
	{
        $localExtensions = [];
...
// ToDo Description
    ...
    {
        return count(
            Tools\Folder::scanForFile($this->repositoryPath,
            filter: 'developer.') ?? []
        );
	}
...
// ToDo recusive move to tools
     ...
     * @since 0.12
     */
    final function getVendors(): array
    {
        $array = [];
            ...
            $existingGroup = $queryBuilder
                ...
                ->where(
                    $queryBuilder->expr()->eq(
                        'uid', $queryBuilder->createNamedParameter((int)$backendGroupId, ParameterType::INTEGER)
                    )
                )
                ->setMaxResults(1)
                ->executeQuery()
